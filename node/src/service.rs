...
use alpha_runtime::{self, opaque::Block, RuntimeApi};
...
pub fn new_partial(
	...
	sc_service::PartialComponents<
		...
		(
			sc_consensus_aura::AuraBlockImport<Block, FullClient, AuraPair>,
...
pub fn new_full(config: Configuration) -> Result<TaskManager, ServiceError> {
	...
	if enable_grandpa {
		...
		let grandpa_params = sc_consensus_grandpa::GrandpaParams {
			...
			link: grandpa_link,
			network,
			sync: Arc::new(sc_consensus_grandpa::warp_proof::NetworkProvider::new(
				backend,
				grandpa_link.shared_authority_set().clone(),
				Vec::default(),
			)),
